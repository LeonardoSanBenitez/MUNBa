...
the ball of radius `√2` (the implicit normalization forced by the paper's own Eq. 22, see the
module docstring), then `gt = α_r • g_r + α_f • g_f` for `α_r := 1/u_r(gt) > 0`,
`α_f := 1/u_f(gt) > 0` — exactly the paper's boxed conclusion. -/
theorem theorem_2_3_optimality_condition {g_r g_f gt : V}
  ...
  exact ⟨hfinal.symm, inv_pos.mpr hr, inv_pos.mpr hf⟩
